    ...
    try {
      ...
      try {
        ...
        return rows.map(row => ({
          ...
          home_id: this.generateHomeId(row.customer_name),
          zone: undefined,
...
7. Generate home_id if missing: HOME_<sanitized_name_uppercase>
...
OUTPUT SCHEMA:
{
  "normalized_rows": [{
    ...
    "zone": "string (optional area/route)",
...
Output: {
  ...
  /**
   * Generate home ID from customer name (fallback only)
   */
