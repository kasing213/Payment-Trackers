...
import { ExcelImportLog, NormalizedRowData } from '../domain/models/excel-import';
...
export class ExcelProcessorWorker {
  ...
  constructor(
    ...
    private changeDueDateCommand: ChangeDueDateCommand,
    private excelImportLogRepository: IExcelImportLogRepository
  ) {}
  ...
  private async processFile(filePath: string): Promise<void> {
      ...
      for (const row of normalizedRows) {
        try {
          ...
          if (!existing) {
            ...
            console.log(`[Excel Processor] Row ${row.row_index}: New AR created for ${row.customer_name}`);
            ...
            if (!hasChanges) {
              ...
              if (changes.due_date_changed) {
                await this.changeDueDateCommand.execute({
                  ar_id: existing.ar_id,
                  new_due_date: row.due_date,
                  reason: `Excel import update: ${changes.changes.join(', ')}`,
                  actor_user_id: 'SYSTEM_EXCEL_IMPORT',
    ...
    try {
      await this.excelImportLogRepository.save(importLog);
    } catch (error: any) {
      console.error('[Excel Processor] Failed to save import log:', error);
    }
